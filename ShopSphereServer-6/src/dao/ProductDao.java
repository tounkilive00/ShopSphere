...
public class ProductDao {
    ...
    // ── Masquer / afficher (reversible, distinct de la suppression) ────────
    public Product setVisibility(int productId, boolean visible) {
        Session ss = null;
        Transaction tr = null;
        try {
            ss = HibernateUtil.getSessionFactory().openSession();
            tr = ss.beginTransaction();
            Product p = (Product) ss.get(Product.class, productId);
            if (p != null) {
    ...
    public List<Product> findActiveProducts() {
        ...
        try {
            ...
            List<Product> products = ss.createQuery(
                    "SELECT p FROM Product p WHERE p.status = :status AND p.visible = true AND p.stockQty > 0 ORDER BY p.id DESC")
    ...
    public List<Product> findByCategory(Category category) {
        ...
        try {
            ss = HibernateUtil.getSessionFactory().openSession();
            List<Product> products = ss.createQuery(
                    "SELECT p FROM Product p WHERE p.category = :cat AND p.status = :status AND p.visible = true ORDER BY p.id DESC")
    ...
    public List<Product> searchByName(String keyword) {
        ...
        try {
            ...
            List<Product> products = ss.createQuery(
                    "SELECT p FROM Product p WHERE p.status = :status AND p.visible = true AND " +
    ...
    public List<Product> findByPriceRange(double min, double max) {
        ...
        try {
            ss = HibernateUtil.getSessionFactory().openSession();
            List<Product> products = ss.createQuery(
                    "SELECT p FROM Product p WHERE p.status = :status AND p.visible = true " +
