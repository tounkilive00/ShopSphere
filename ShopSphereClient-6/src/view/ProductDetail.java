...
public class ProductDetail extends JFrame {
    ...
    public ProductDetail(User user, Product product) {
        ...
        PageHeader header = new PageHeader(product.getTitle());
        ...
        // Prix
        if (product.isOnSale()) {
            JLabel sale = new JLabel(String.format("%.2f FCFA", product.getSalePrice()));
            sale.setFont(new Font("Arial", Font.BOLD, 26));
            sale.setForeground(Theme.ERROR);
            info.add(sale);
            JLabel orig = new JLabel(String.format("Prix normal : %.2f FCFA", product.getBasePrice()));
            orig.setFont(Theme.FONT_SMALL);
            orig.setForeground(Theme.GREY_TEXT);
            info.add(orig);
        } else {
            JLabel price = new JLabel(String.format("%.2f FCFA", product.getBasePrice()));
